...
client.on('message_create', async (msg) => {
  if (msg.body === '!everyone') {  // "!everyone" will mention everyone in a group chat. Works seamlessly with no traffic problems.
    ...
    if (chat.isGroup) { 
        ...
        await chat.sendMessage("This command can only be used in group chats.");
    }
  }
      ...
      const meme = await axios("https://meme-api.com/gimme") //Link to scrape memes from Reddit. Generally takes a bit of time because of Traffic. Please wait.
      .then(res => res.data);
